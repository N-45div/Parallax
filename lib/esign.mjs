 ...
 *
 * The bytes go up as base64 rather than as a URL. Handing eSign a link means
 * only files we already host can ever be signed, which quietly limited the whole
 * handoff to our own two fixtures — anyone testing their own document never saw
 * the part of this that matters.
 *
 * @param {object} opts
 * @param {Buffer|Uint8Array} opts.bytes   the cleared document itself
...
export async function sendForSignature(opts, env) {
  ...
  const res = await fetch(`${BASE}/esign/api/v1/folders/createfolder`, {
    ...
    body: JSON.stringify({
      ...
      inputType: 'base64',
      base64FileString: [Buffer.from(opts.bytes).toString('base64')],
  ...
  // A malformed request still comes back 200 with nothing attached. Reporting
  // that as a successful handoff would be a lie in the demo's favour.
  if (!folderId) {
    throw new Error('Foxit eSign accepted the request but returned no folder, so nothing is actually waiting on a signature.');
  }
