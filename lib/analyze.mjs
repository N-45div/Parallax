...
function verdict({ concealed, findings, evidence }) {
  ...
  return { decision: 'SIGN', reason: 'All four readings agree and no concealed content was found.' };
}
...
export async function analyze(bytes, env, { withEvidence = true } = {}) {
  ...
  return {
    views, nutrient: { text: ntext, structure }, findings, leaked,
    ...
    evidence,
    verdict: verdict({ concealed: views.concealed, findings, evidence }),
  };
}
