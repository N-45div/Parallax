...
function verdict({ concealed, findings, domain, runs }) {
  ...
  const mismatches = findings.filter((f) =>
    /only in concealed|different jurisdiction|IBAN is registered|resolves to .* in the public registry/i.test(f)).length;
...
export async function analyze(bytes, env, { withEvidence = true } = {}) {
  ...
  // Evidence that cannot change the answer is decoration. The registry lookup is
  // the one claim on an invoice with a single correct answer: a company number
  // identifies exactly one company, so a result naming a different one is a flat
  // contradiction of the letterhead rather than a soft signal.
  const registry = evidence.find((c) => c.id === 'company_no');
  if (registry?.results?.length) {
    const vendorWords = String(fieldsFromStructure(structure, views.viewB).vendor_name ?? '')
      .toLowerCase().split(/[^a-z0-9]+/).filter((w) => w.length > 3);
    const named = registry.results
      .map((r) => `${r.title} ${r.snippet}`)
      .join(' ');
    const mentionsVendor = vendorWords.length > 0 && vendorWords.some((w) => named.toLowerCase().includes(w));
    const otherCompany = named.match(/([A-Z][A-Za-z.&' ]{3,40}?(?:LTD|LIMITED|PLC|LLP))/);
    if (!mentionsVendor && otherCompany) {
      findings.push(
        `Company number ${registry.value} resolves to ${otherCompany[1].trim()} in the public registry, ` +
        `not to the supplier named on this letterhead.`);
    }
  }
  ...
  if (claimedDomain && env.NAMECOM_TOKEN && env.NAMECOM_USERNAME) {
    try {
      ...
      if (domain.verdict === 'unregistered') {
        ...
        if (domain.suggested?.cheapest) {
          findings.push(
            `A convincing lookalike of this supplier is on sale right now: ` +
            `${domain.suggested.cheapest.domain} for $${domain.suggested.cheapest.price}. ` +
            `That is the cost of impersonating this vendor, quoted by a registrar.`);
        }
