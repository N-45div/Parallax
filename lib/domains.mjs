...
/**
 * The registrar's own confusable engine.
 *
 * `neighbours()` composes transformations we thought of. A registrar has spent
 * years learning which strings people actually mistake for one another, and it
 * returns names our generator structurally cannot reach — a singular for a
 * plural, a "my" prefix, an "inc" that was never in the original. Asking it is
 * strictly more coverage than reasoning harder about permutations.
 *
 * It also prices what it offers, which converts a list of lookalikes into the
 * one number a finance reader actually weighs: what it costs to buy one.
 */
export async function searchConfusables(label, env, tlds = ['com', 'co.uk', 'net', 'io', 'co']) {
  const res = await call('/domains:search', { keyword: label, tldFilter: tlds }, env);
  const results = (res.results ?? []).map((r) => ({
    domain: String(r.domainName).toLowerCase(),
    registered: r.purchasable !== true,
    price: typeof r.purchasePrice === 'number' ? r.purchasePrice : null,
  }));
  const cheapest = results
    .filter((r) => !r.registered && r.price != null)
    .sort((a, b) => a.price - b.price)[0] ?? null;
  return { results, taken: results.filter((r) => r.registered), cheapest };
}
...
export async function inspectDomain(domain, env) {
  ...
  // The registrar's suggestions are additive: anything it flags as taken that we
  // never generated is a lookalike our own permutation rules could not reach.
  let suggested = null;
  try {
    const { label } = split(domain);
    const found = await searchConfusables(label.replace(/-/g, ''), env);
    const known = new Set(candidates.map((d) => d.toLowerCase()));
    suggested = {
      extraTaken: found.taken.filter((r) => !known.has(r.domain)).map((r) => r.domain),
  ...
  return { domain, registered, verdict, checked: candidates.length, neighbours: taken, cores, suggested, note };
}
